...
manual_matches <- tribble(
  ...
  # Argentina
  810207,   424342,  "Argentina",           # Jeremías Lázaro -> Jeremías Ezequiel Lázaro (Instituto)
  # Chile
  922030,   443560,  "Chile",               # Felipe Faúndez -> F.González (O'Higgins)
  741346,   463019,  "Chile",               # Lucas Molina -> L.Molina Canto (Ñublense)
  # LaLiga
  117268,   359115,  "LaLiga",              # Andrés López Gallo -> Andrés Castrin (Sevilla)
  13519,    6587,    "LaLiga",              # Antonio Latorre Grueso -> Toni Lato (Mallorca)
  53346,    39538,   "LaLiga",              # Gerard Fernández Castellano -> Peque (Sevilla)
  892503,   419557,  "LaLiga",              # Joaquin Martínez Gauna -> Oso (Sevilla)
  29176,    30054,   "LaLiga",              # José Luis García Vayá -> Pepelu (Valencia)
  ...
  30948,    29237,   "Liga MX",             # Robson Alves de Barros -> Robson Bambu (Atlético San Luis)
  28353,    25342,   "Liga MX",             # Helio Júnio Nunes de Castro -> Helinho (Toluca)
  8496,     18784,   "Liga MX",             # João Paulo Dias Fernandes -> Paulinho (Toluca)
  43279,    114730,  "Liga MX",             # Luiz Eduardo Teodora da Silva -> Dudu (Mazatlán)
  25260,    44068,   "Liga MX",             # Nathanael Ananias da Silva -> Nathan Silva (Pumas)
  49308,    123690,  "Liga MX",             # Vitor Samuel Ferreira Arantes -> Vitinho (Tijuana)
  # MLS (new)
  31171,    16761,   "MLS",                 # Alexander Roldán Leon -> Alex Roldán (Seattle)
  31134,    12573,   "MLS",                 # José Guillermo Rodríguez -> Memo Rodríguez (Sporting KC)
  25477,    13449,   "MLS",                 # Kee-Hee Kim -> Kim Ki-Hee (Seattle)
  7157,     3027,    "MLS",                 # Mathias Jattah-Njie Jørgensen -> Zanka (LA Galaxy)
  31520,    25349,   "MLS",                 # Ocimar de Almeida Júnior -> Júnior Urso (Houston)
  171787,   271465,  "MLS",                 # Pedro Henrique Fonseca de Araújo Martins -> Pedrinho (FC Dallas)
  141706,   230617,  "MLS",                 # Rwan Philipe Rodrigues de Souza Cruz -> Rwan Seco (Real Salt Lake)
  10373,    18242,   "MLS",                 # Timothy Michael Ream -> Tim Ream (Charlotte)
  # Premier League
  44869,    39241,   "Premier League",      # Valentino Francisco Livramento -> Tino Livramento (Newcastle)
  # Brasil (new nickname matches)
  31535,    25764,   "Brasil – Série A",    # Anderson Luis de Carvalho -> Nenê (Juventude)
  31671,    25245,   "Brasil – Série A",    # Caio Fernando de Oliveira -> Caio Paulista (Atlético Mineiro)
  3921,     27118,   "Brasil – Série A",    # Carlos Gilberto Nascimento Silva -> Gil (Santos)
  31591,    27701,   "Brasil – Série A",    # Carlos de Menezes Júnior -> Cacá (Corinthians)
  172955,   126449,  "Brasil – Série A",    # Claudio Coelho Salvático -> Claudinho (EC Vitória)
  31634,    26473,   "Brasil – Série A",    # Claudio Rodrigues Gomes -> Guga (Fluminense)
  174160,   232339,  "Brasil – Série A",    # Cleidson Andrade de Souza Silva -> Camutanga (EC Vitória)
  12397,    20824,   "Brasil – Série A",    # Danilo das Neves Pinheiro -> Tchê Tchê (Vasco)
  31704,    31954,   "Brasil – Série A",    # Douglas Moreira Fagundes -> Dodi (Grêmio)
  24856,    25123,   "Brasil – Série A",    # Eduardo Pereira Rodrigues -> Dudu (Atlético Mineiro)
  23980,    25327,   "Brasil – Série A",    # Eric dos Santos Rodrigues -> Ramires (Red Bull Bragantino)
  25176,    25159,   "Brasil – Série A",    # Erick Luis Conrado Carvalho -> Erick (Bahia)
  31644,    26557,   "Brasil – Série A",    # Felipe Costa -> Chico (Mirassol)
  13476,    18977,   "Brasil – Série A",    # Francisco das Chagas Soares dos Santos -> Tiquinho (Santos)
  25184,    25178,   "Brasil – Série A",    # Glaybson Yago Souza Lisboa -> Yago Pikachu (Fortaleza)
  828621,   435294,  "Brasil – Série A",    # Guilherme William Silva Inacio -> Gui Negão (Corinthians)
  32891,    44723,   "Brasil – Série A",    # Gustavo Henric da Silva -> Gustavo Mosquito (EC Vitória)
  978489,   436677,  "Brasil – Série A",    # Gustavo Teixeira Lopes da Conceição -> Gustavo Mancha (Fortaleza)
  43397,    113297,  "Brasil – Série A",    # Igor Aquino da Silva -> Igor Cariús (Sport Recife)
  142916,   170305,  "Brasil – Série A",    # Jhonatan dos Santos Rosa -> Jhon Jhon (Red Bull Bragantino)
  1005311,  473805,  "Brasil – Série A",    # José Lucas Gomes da Silva -> Zé Lucas (Sport Recife)
  28283,    25171,   "Brasil – Série A",    # José Romário Silva de Souza -> Romarinho (Sport Recife)
  31767,    21236,   "Brasil – Série A",    # João Pedro Vilardi Pinto -> Pepê (EC Vitória)
  439492,   271717,  "Brasil – Série A",    # João Pedro de Sousa Rodrigues -> Tchoca (Corinthians)
  31657,    25259,   "Brasil – Série A",    # Luis Antonio da Rocha Júnior -> Juninho Capixaba (Red Bull Bragantino)
  301488,   271556,  "Brasil – Série A",    # Luis Eduardo Soares da Silva -> Cuiabano (Botafogo)
  181600,   395580,  "Brasil – Série A",    # Mateus Carvalho dos Santos -> Mateus Cocão (Vasco)
  25264,    129800,  "Brasil – Série A",    # Matheus Alexandre Anastácio de Souza -> Matheus Alexandre (Sport)
  175854,   211232,  "Brasil – Série A",    # Matheus Lima Beltrão Oliveira -> Matheus Bidu (Corinthians)
  633848,   196863,  "Brasil – Série A",    # Matheus Martins Fogaça de Paula -> Matheuzinho (EC Vitória)
  28342,    25527,   "Brasil – Série A",    # Mário Sérgio Santos Costa -> Marinho (Fortaleza)
  29932,    24016,   "Brasil – Série A",    # Victor Alexander da Silva -> Vitinho (Botafogo)
  748003,   405042,  "Brasil – Série A",    # Vinicius Mendonça Pereira -> Vinicinho (Red Bull Bragantino)
  30705,    44089,   "Brasil – Série A",    # Vitor Eduardo da Silva Matos -> Vitão (Internacional)
  25174,    26923,   "Brasil – Série A"     # Vitor Hugo Naum dos Santos -> Vitinho (Internacional)
) |>
  mutate(
