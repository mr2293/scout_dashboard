...
tm_crosswalk <- if (file.exists(tm_crosswalk_path)) {
  read.csv(tm_crosswalk_path, stringsAsFactors = FALSE, colClasses = "character") |>
    ...
    dplyr::select(player_name, team_name, tm_player_id, market_value_raw, market_value_eur, contract_expires, player_agent) |>
  ...
  data.frame(
    player_name = character(), team_name = character(), tm_player_id = character(),
    market_value_raw = character(), market_value_eur = numeric(),
    contract_expires = character(), player_agent = character(),
...
get_all_players_sc_df <- function() {
  ...
  .all_players_sc_df_cache <<- result
  .all_players_sc_df_cache
}
...
get_all_sc_df <- function() {
  if (is.null(.all_sc_df_cache)) {
    ...
    .all_sc_df_cache <<- dplyr::bind_rows(dfs)
  }
  .all_sc_df_cache
}
...
server <- function(input, output, session) {
  ...
  sc_table_data <- reactive({
    ...
    message(sprintf("[SC_REACTIVE] liga_mx_sc_df rows=%d  all_sc_df rows=%d",
                    nrow(get_liga_mx_sc_df()), nrow(get_all_sc_df())))
